...
isCornerPiece b borders = (length $ filter (\(tns, rns, bns, lns) -> tns == [] && lns == []) $ map (\s -> neighbourCands s bs') bsym) > 0
    where bs'   = deleteBy (\b b' -> (getId b) == (getId b')) b borders
          bsym  = symmetries b
...
main = do
    ...
    cornerPieces <- return $ filter ((flip isCornerPiece) borders) borders
    putStrLn $ show cornerPieces
    putStrLn $ show $ findIndex (\b -> getId b == 3229) borders 
    return $ product $ map getId cornerPieces
